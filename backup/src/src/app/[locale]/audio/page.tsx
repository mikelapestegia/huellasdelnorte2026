...
export default function AudioPage() {
    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-200">
            {/* Hero */}
            <div className="relative pt-32 pb-16 px-4 bg-gradient-to-b from-slate-900 to-slate-950 border-b border-white/5">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                    ...
                    <div className="flex justify-center gap-4 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                        <button
                            ...
                            className={`px-6 py-2 rounded-full font-medium transition-all ${activeTab === 'all' ? 'bg-violet-600 text-white shadow-lg shadow-violet-900/50' : 'bg-slate-800 text-slate-400 hover:text-white'
                                }`}
                        >
                            Todo
                        </button>
                        <button
                            ...
                            className={`px-6 py-2 rounded-full font-medium transition-all flex items-center gap-2 ${activeTab === 'Audiolibro' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/50' : 'bg-slate-800 text-slate-400 hover:text-white'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-6 py-2 rounded-full font-medium transition-all flex items-center gap-2 ${activeTab === 'Podcast' ? 'bg-rose-600 text-white shadow-lg shadow-rose-900/50' : 'bg-slate-800 text-slate-400 hover:text-white'
                                }`}
                        >
            ...
            <div className="max-w-6xl mx-auto px-4 py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {filteredAudio.map((item) => (
                        <div key={item.id} className="bg-slate-900 rounded-2xl overflow-hidden border border-white/5 hover:border-violet-500/30 transition-all shadow-xl">
                            {/* Card Header */}
                            <div className="flex p-6 gap-6 items-start">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-400 mb-2">Por {item.author}</p>
            ...
            {/* Footer Notice */}
            <div className="border-t border-white/5 bg-slate-950 py-12 px-4">
                <div className="max-w-3xl mx-auto text-center">
                    <h4 className="text-sm font-bold text-slate-400 mb-4 uppercase tracking-wider flex items-center justify-center gap-2">
...
function AudioLicenseTemplate({ item }: { item: AudioContent }) {
    if (item.licenseType === 'public_domain') {
        return (
            <div className="text-[10px] text-slate-500 font-mono space-y-1 bg-slate-900/50 p-3 rounded border border-white/5">
                <p><span className="text-slate-400 font-bold">Título:</span> {item.title}</p>
                <p><span className="text-slate-400 font-bold">Fuente:</span> Grabación de Dominio Público cortesía de {item.source}.</p>
                <p><span className="text-slate-400 font-bold">Licencia:</span> Este material es libre de derechos de autor y se distribuye bajo licencia de Dominio Público.</p>
                {item.narrator && <p><span className="text-slate-400 font-bold">Narrado por:</span> {item.narrator} (Comunidad LibriVox)</p>}
            </div>
        );
    }
    ...
    if (item.licenseType === 'cc_by') {
        return (
            <div className="text-[10px] text-slate-500 font-mono space-y-1 bg-slate-900/50 p-3 rounded border border-white/5">
                <p className="font-bold text-slate-400 mb-1">Créditos:</p>
    ...
    if (item.licenseType === 'embed') {
        return (
            <div className="text-[10px] text-slate-500 font-mono bg-slate-900/50 p-3 rounded border border-white/5">
                <p>
