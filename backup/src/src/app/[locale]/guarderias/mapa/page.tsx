        ...
        map.current.on('load', () => {
            ...
            allKennels.forEach((kennel) => {
                ...
                el.addEventListener('click', (event) => {
                    ...
                    map.current?.flyTo({
                        center: [kennel.coordinates!.lng, kennel.coordinates!.lat],
                        zoom: 12,
                        essential: true
                    });
                });
                ...
                new maplibregl.Marker({ element: el })
                    .setLngLat([kennel.coordinates.lng, kennel.coordinates.lat])
                    .addTo(map.current!);

                bounds.extend([kennel.coordinates.lng, kennel.coordinates.lat]);
            });
        ...
        <div className="relative w-full h-screen bg-slate-900">
            ...
            {!mapReady && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-900/60 z-10">
                    <div className="bg-slate-900/80 border border-white/10 rounded-xl px-4 py-2 text-sm text-slate-300">
                        Cargando mapa...
                    </div>
                </div>
            )}
            ...
            {selectedKennel && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-sm px-4 z-20 animate-in slide-in-from-bottom-5 fade-in duration-300">
                    <div className="bg-slate-900/90 backdrop-blur-xl border border-white/20 rounded-2xl p-5 shadow-2xl relative">
                        <button
