    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-200 pb-20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
                {/* Search & Filter Bar */}
                <div className="bg-slate-900 rounded-2xl p-4 shadow-xl border border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between mb-12">
                    <div className="relative w-full md:w-96">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2.5 pl-10 pr-9 text-white focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all placeholder:text-slate-600"
                        />
                        {searchQuery && (
                            <button
                                type="button"
                                ...
                                className="absolute right-2 top-2 p-1 rounded-full text-slate-500 hover:text-white hover:bg-white/10 transition-colors"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredEvents.map((event, index) => (
                        <div
                            key={event.id}
                            className="group bg-slate-900 rounded-2xl overflow-hidden border border-white/5 hover:border-indigo-500/30 transition-all hover:shadow-2xl hover:shadow-indigo-900/20 flex flex-col animate-fade-in-up"
                        ...
                        >
                            <div className="aspect-video relative overflow-hidden">
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1541599540903-216a46ca1dc0?q=80&w=2671&auto=format&fit=crop";
                                    }}
                                />
                                <div className="absolute top-4 right-4 bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-lg border border-white/10 text-xs font-bold text-white shadow-lg">
                                    {event.month}
                                </div>
                                <div className={`absolute top-4 left-4 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider shadow-lg ${event.type === 'Pastor' ? 'bg-amber-500 text-slate-900' :
                                    event.type === 'Mushing' ? 'bg-sky-500 text-slate-900' :
                                        event.type === 'Agility' ? 'bg-emerald-500 text-slate-900' :
                            ...
                            <div className="p-6 flex-1 flex flex-col">
                                ...
                                <div className="flex items-center gap-2 text-sm text-slate-400 mb-4">
                                    ...
                                    <span>{event.location} <span className="text-slate-600">•</span> {event.region}</span>
                                </div>
                    ...
                    {filteredEvents.length === 0 && (
                        <div className="col-span-full py-20 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-500">No se encontraron eventos</h3>
                            <p className="text-slate-600">Intenta cambiar los filtros de búsqueda.</p>
                            {hasActiveFilters && (
                                <button
                                    type="button"
                                    onClick={clearFilters}
                ...
                {/* Social Meetups CTA */}
                <div className="mt-16 bg-gradient-to-r from-emerald-900/40 to-slate-900 border border-emerald-500/20 rounded-2xl p-8 md:p-12 text-center relative overflow-hidden group">
