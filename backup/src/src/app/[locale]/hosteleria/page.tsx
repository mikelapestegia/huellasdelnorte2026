        ...
        <main className="min-h-screen bg-slate-950 text-slate-200">
            ...
            <div className="relative h-[400px] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-20 text-center px-4 max-w-4xl mx-auto">
                    ...
                    <p className="text-lg text-slate-300 max-w-2xl mx-auto animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                ...
                {/* Filters */}
                <div className="flex flex-col md:flex-row gap-6 mb-6 items-center justify-between bg-slate-900/50 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch gap-3 w-full md:w-auto">
                        <div className="relative w-full md:w-64">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900 border border-white/10 rounded-xl py-2.5 pl-9 pr-9 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                            />
                            {searchQuery && (
                                <button
                                    type="button"
                                    ...
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full text-slate-500 hover:text-white hover:bg-white/10 transition-colors"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                            )}
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <span className="text-sm text-slate-400 hidden md:inline">Región:</span>
                            <select
                                value={selectedRegion}
                                onChange={(e) => setSelectedRegion(e.target.value)}
                                aria-label="Filtrar por región"
                                className="bg-slate-800 border-none text-white text-sm rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 w-full md:w-48 cursor-pointer hover:bg-slate-700 transition-colors"
                            >
                ...
                {hasActiveFilters && (
                    <div className="mb-10 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredPlaces.length > 0 ? (
                        filteredPlaces.map((place) => (
                            <div
                                key={place.id}
                                className="group relative bg-slate-900 rounded-3xl overflow-hidden border border-white/5 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-900/10 flex flex-col"
                            >
                                {/* Image Placeholder */}
                                <div className="h-48 bg-slate-800 relative overflow-hidden">
                                    ...
                                    <div className="absolute top-4 right-4 z-20 bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 text-xs font-bold text-white uppercase tracking-wider">
                                        {place.city}
                                    </div>
                                    <div className="absolute bottom-4 left-4 z-20 flex gap-2">
                                        ...
                                        <span className="bg-slate-900/90 backdrop-blur-md px-2 py-1 rounded-lg text-xs font-medium text-slate-300 border border-white/10">
                                            {place.priceRange}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    {place.features && (
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {place.features.map(feature => (
                                                <span key={feature} className="text-[10px] uppercase font-bold text-slate-500 bg-slate-800 px-2 py-1 rounded-md">
                                                    {feature}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 pt-4 border-t border-white/5">
                                        {place.website && (
                                            <a
                                                ...
                                                className="flex items-center justify-center gap-2 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 text-xs font-medium transition-colors"
                                            >
                        ...
                        <div className="col-span-full py-20 text-center">
                            <p className="text-slate-500 text-lg">No se encontraron lugares con estos filtros.</p>
                            <button
                                type="button"
                                onClick={clearFilters}
