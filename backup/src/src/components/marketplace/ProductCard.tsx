...
export function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <div className="bg-slate-900 rounded-2xl overflow-hidden border border-white/5 hover:border-emerald-500/30 transition-all hover:shadow-xl group flex flex-col h-full">
            {/* Image Container */}
                    ...
                    {product.monetization === 'subscription' && product.subscriptionDetails ? (
                        <div className="space-y-3">
                            <div className="flex items-end gap-2">
                                ...
                                <span className="text-xs text-slate-400 mb-1">/ mes</span>
                            </div>
                            <p className="text-xs text-slate-500">Dispositivo desde {product.subscriptionDetails.devicePrice}€</p>
                        ...
                        <div className="space-y-3">
                            ...
                            {availableShops.length > 0 ? (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-1 overflow-x-auto pb-1">
                                        {availableShops.map(shop => (
                                            <span key={shop.id} className="text-[10px] bg-slate-800 border border-slate-700 px-2 py-1 rounded-full whitespace-nowrap text-slate-300">
                                                {shop.city}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <button disabled className="w-full py-2.5 rounded-xl bg-slate-800 text-slate-500 font-bold text-sm cursor-not-allowed">
                        ...
                        <div className="space-y-3">
                            ...
                            <a
                                ...
                                className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm transition-colors flex items-center justify-center gap-2"
                            >
