...
export default function VeterinariosPage() {
    ...
    return (
        <main className="min-h-screen bg-slate-950 text-slate-200">
            ...
            <div className="bg-red-500/10 border-b border-red-500/20 pt-32 pb-8 px-4">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-red-200/80 max-w-2xl mx-auto text-sm md:text-base">
                        Listado de clínicas veterinarias y hospitales de referencia en el norte de España.
            ...
            {/* Controls */}
            <div className="sticky top-16 z-30 bg-slate-950/80 backdrop-blur-md border-b border-white/5 py-4 px-4">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
                    ...
                    <div className="relative w-full md:w-72">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-900 border border-white/10 rounded-lg py-2 pl-9 pr-9 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                        />
                        {searchQuery && (
                            <button
                                type="button"
                                ...
                                className="absolute right-2 top-1.5 p-1 rounded-full text-slate-500 hover:text-white hover:bg-white/10 transition-colors"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        )}
                ...
                <div className="max-w-7xl mx-auto mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
            ...
            <div className="max-w-5xl mx-auto px-4 py-12 space-y-12">
                ...
                {filteredClinics.length === 0 && (
                    <div className="text-center py-20 bg-slate-900/50 rounded-2xl border border-dashed border-white/10">
                        ...
                        <p className="text-slate-400">No se encontraron clínicas con estos criterios.</p>
                        <button
                            type="button"
                            onClick={clearFilters}
...
function ClinicCard({ clinic, priority = false }: { clinic: VeterinaryClinic, priority?: boolean }) {
    ...
    return (
        <div className={`relative group rounded-2xl p-6 transition-all duration-300 ${priority
                ? 'bg-slate-900 border border-emerald-500/30 hover:border-emerald-500 shadow-xl shadow-emerald-900/10'
                : 'bg-slate-900/50 border border-white/5 hover:border-amber-500/30'
            }`}>
            <div className="flex justify-between items-start mb-4">
                <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${is24h ? 'bg-emerald-500 text-slate-900' : 'bg-amber-500/20 text-amber-400 border border-amber-500/20'
                    }`}>
                    {clinic.type}
                </div>
            ...
            <p className="text-sm text-slate-500 mb-6 border-b border-white/5 pb-4">
                {clinic.address}
            </p>
