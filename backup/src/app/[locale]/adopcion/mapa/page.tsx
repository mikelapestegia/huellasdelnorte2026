        ...
        map.current.on('load', () => {
            ...
            dogsWithCoords.forEach((dog) => {
                ...
                el.addEventListener('click', () => {
                    setSelectedDog(dog);
                    map.current?.flyTo({
                        center: [dog.coordinates.lng, dog.coordinates.lat],
                        zoom: 10,
                        essential: true
                    });
                });
        ...
        <div className="relative w-full h-screen bg-slate-900">
            ...
            <div className="absolute top-0 left-0 right-0 z-10 p-4 bg-gradient-to-b from-slate-900/80 to-transparent pointer-events-none">
                <div className="max-w-7xl mx-auto flex items-center justify-between pointer-events-auto">
                    <Link
                        href="/adopcion"
                        className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 hover:bg-slate-800 text-white rounded-xl backdrop-blur-md border border-white/10 transition-colors"
                    >
                        <ArrowLeft className="w-5 h-5" />
                        <span className="hidden sm:inline">Volver al listado</span>
                    </Link>

                    <div className="bg-slate-900/50 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 text-white font-medium flex items-center gap-2">
            ...
            <div className="absolute top-20 left-1/2 -translate-x-1/2 z-10 px-4 w-full max-w-5xl pointer-events-none">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div className="flex flex-wrap items-center gap-3 pointer-events-auto">
                        <div className="bg-slate-900/80 border border-white/10 rounded-full px-4 py-2 text-xs text-slate-200">
                        ...
                        <div className="bg-slate-900/80 border border-white/10 rounded-full px-4 py-2 text-xs text-slate-300">
                            Verde: disponible · Rojo: urgente
                        </div>
                    </div>
                    <button
                        onClick={resetView}
                        className="pointer-events-auto bg-slate-900/70 border border-white/10 rounded-full px-4 py-2 text-xs text-slate-200 hover:bg-slate-800 transition-colors"
                    >
                        Centrar mapa
                    </button>
                </div>
            ...
            {!mapLoaded && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm z-10">
                    <div className="text-slate-200 text-sm">Cargando mapa…</div>
                </div>
            )}

            {mapLoaded && dogsWithCoords.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center z-10">
                    <div className="bg-slate-900/80 border border-white/10 rounded-2xl px-6 py-4 text-center text-slate-200 max-w-sm">
            ...
            {selectedDog && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-sm px-4 z-20 animate-in slide-in-from-bottom-5 fade-in duration-300">
                    <div className="bg-slate-900/90 backdrop-blur-xl border border-white/20 rounded-2xl p-4 shadow-2xl relative">
                        <button
                            onClick={() => setSelectedDog(null)}
                            aria-label="Cerrar"
                            className="absolute top-2 right-2 p-1.5 hover:bg-white/10 rounded-full text-slate-400 hover:text-white transition-colors"
                        >
                            <X className="w-5 h-5" />
                        </button>
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-slate-300 text-sm mb-1">{selectedDog.breed}</p>
