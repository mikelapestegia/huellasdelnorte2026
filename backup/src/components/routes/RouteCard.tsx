...
export default function RouteCard({ route, onClick }: RouteCardProps) {
    ...
    return (
        <Link
            href={`/rutas/${route.id}`}
            onClick={onClick}
            className="block w-full text-left rounded-2xl border border-border bg-white shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group"
        >
            ...
            <div className="relative h-40 bg-gradient-to-br from-primary/20 to-accent/20 overflow-hidden">
                ...
                {/* Region badge */}
                <div className="absolute top-3 left-3 px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-sm text-xs font-semibold text-foreground">
                    {route.region}
                </div>
                ...
                {/* Route type icon */}
                <div className="absolute bottom-3 right-3 h-10 w-10 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-primary font-bold text-xs">
