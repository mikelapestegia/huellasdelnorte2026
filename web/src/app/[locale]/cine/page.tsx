...
export default function CinemaPage() {
    ...
    return (
        <main className="min-h-screen bg-background text-foreground selection:bg-rose-500/30">
            {/* Hero Section */}
            <div className="relative pt-32 pb-16 px-4 bg-gradient-to-br from-card via-rose-950/20 to-background">
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    ...
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8 leading-relaxed animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            ...
            <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="flex items-center justify-between border-b border-white/5 pb-4">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm transition-colors ${filterStatus === 'all' ? 'bg-card text-foreground font-bold' : 'text-muted-foreground hover:text-white'}`}
                            >
                                Todas
                            </button>
                            <button
                                onClick={() => setFilterStatus('Dominio Público')}
                                className={`px-4 py-2 rounded-lg text-sm transition-colors ${filterStatus === 'Dominio Público' ? 'bg-emerald-500/20 text-emerald-400 font-bold' : 'text-muted-foreground hover:text-white'}`}
                            >
                                Disponibles
                            </button>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {filteredMovies.map((movie) => (
                            <div
                                key={movie.id}
                                className="group bg-card rounded-2xl overflow-hidden border border-white/5 hover:border-rose-500/30 transition-all hover:shadow-2xl hover:shadow-rose-900/10 flex flex-col"
                            >
                                <div className="aspect-video bg-black relative overflow-hidden">
                                    ...
                                    <div className={`absolute top-2 right-2 px-2 py-1 rounded text-xs font-bold border ${movie.status === 'Dominio Público'
                                        ? 'bg-emerald-500 text-background border-emerald-400'
                                ...
                                <div className="p-6 flex-1 flex flex-col">
                                    ...
                                    {movie.notes && (
                                        <div className="bg-card/50 p-3 rounded-lg border border-white/5 mb-4">
                                    ...
                                    {movie.archiveUrl ? (
                                        ...
                                        <button disabled className="mt-auto w-full py-3 rounded-xl bg-card/80 text-muted-foreground font-bold flex items-center justify-center gap-2 cursor-not-allowed border border-white/5">
                ...
                <div className="space-y-8">
                    <div className="bg-card/50 backdrop-blur-md rounded-2xl p-6 border border-white/5 sticky top-24">
