...
export default function HomeMap() {
            ...
            adoptionDogs.forEach(dog => {
                ...
                const marker = new maplibregl.Marker({ element: el })
                    ...
                    .setPopup(new maplibregl.Popup({ offset: 10, closeButton: false }).setHTML(
                        `<div class="p-2 text-background font-bold text-sm">${dog.name}</div>`
                    ))
                    .addTo(map.current!);
        ...
        if (layers.kennels) {
            allKennels.forEach(kennel => {
                ...
                const marker = new maplibregl.Marker({ element: el })
                    ...
                    .setPopup(new maplibregl.Popup({ offset: 10, closeButton: false }).setHTML(
                        `<div class="p-2 text-background font-bold text-sm">${kennel.name}</div>`
                    ))
                    .addTo(map.current!);
    ...
    return (
        <div className="relative w-full h-[600px] rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-card">
            <div ref={mapContainer} className="w-full h-full" />

            {/* Control Panel Overlay */}
            <div className="absolute top-6 left-6 z-10 w-64 bg-card/90 backdrop-blur-md rounded-2xl p-5 border border-white/10 shadow-xl">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs">Filtrar visualización</p>
                    </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <Dog className={`w-4 h-4 ${layers.adoption ? 'text-emerald-400' : 'text-muted-foreground'}`} />
                            <span className={`text-sm font-medium ${layers.adoption ? 'text-white' : 'text-muted-foreground'}`}>Adopción</span>
                        </div>
                        <div className={`w-2 h-2 rounded-full transition-colors ${layers.adoption ? 'bg-emerald-400' : 'bg-card/60'}`} />
                    </button>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <Hotel className={`w-4 h-4 ${layers.kennels ? 'text-amber-400' : 'text-muted-foreground'}`} />
                            <span className={`text-sm font-medium ${layers.kennels ? 'text-white' : 'text-muted-foreground'}`}>Alojamientos</span>
                        </div>
                        <div className={`w-2 h-2 rounded-full transition-colors ${layers.kennels ? 'bg-amber-400' : 'bg-card/60'}`} />
                    </button>
                </div>
            ...
            <div className="absolute top-6 right-6 z-10 hidden md:block text-right pointer-events-none">
                <h2 className="text-3xl font-bold text-white drop-shadow-md">Norte peninsular</h2>
                <p className="text-muted-foreground text-sm drop-shadow-md">Ecosistema canino activo</p>
            </div>
        </div>
    );
}
