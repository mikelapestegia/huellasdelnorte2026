...
export default function DogProfileCard({ dog, isSelected, onSelect, onEdit }: DogProfileCardProps) {
    ...
    return (
        <div
            className={`relative rounded-2xl border-2 bg-card shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden cursor-pointer ${isSelected ? "border-primary ring-2 ring-primary/20" : "border-border"
                }`}
            onClick={onSelect}
        >
            {/* Header with photo */}
            <div className="relative h-32 bg-gradient-to-br from-primary/20 to-accent/20">
                {dog.photoUrl ? (
                    <Image src={dog.photoUrl} alt={dog.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                ) : (
                ...
                {/* Avatar badge */}
                <div className="absolute -bottom-8 left-4 h-16 w-16 rounded-full bg-card border-4 border-border shadow-md flex items-center justify-center">
                    {dog.photoUrl ? (
                        <Image src={dog.photoUrl} alt={`Foto de ${dog.name}`} fill className="rounded-full object-cover" sizes="64px" />
                    ) : (
