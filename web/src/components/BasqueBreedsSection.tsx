...
import basqueBreedsData from '@/data/basque_breeds.json';
...
const BasqueBreedsSection = () => {
  const [breeds, setBreeds] = useState<BreedInfo[]>([]);

  useEffect(() => {
    // In a real app, we would fetch from an API
    // For now, we'll use the static data
    setBreeds(basqueBreedsData.basque_breeds as BreedInfo[]);
  }, []);

  return (
    <section className="basque-breeds-section">
      <h2>Razas Autóctonas del País Vasco</h2>
      <p>Descubre las razas de perros originarias del País Vasco, adaptadas a las condiciones climáticas y geográficas de la región.</p>
      ...
      <div className="breed-info-sources">
        ...
        <ul>
          {basqueBreedsData.breed_info_sources?.map((source, idx) => (
            <li key={idx}>
...
export default BasqueBreedsSection;